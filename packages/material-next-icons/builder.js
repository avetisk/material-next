...
const SVG_ICON_ABSOLUTE_REQUIRE = '@material-next/core/SvgIcon';
...
function parseArgs() {
  return yargs
    ...
    .describe(
      'file-suffix',
      'Filter only files ending with a suffix (pretty much only for @material-next/icons)',
    )
    .describe(
      'rename-filter',
    ...
    .options('mui-require', {
      demand: false,
      type: 'string',
      describe: `Load material-next dependencies (SvgIcon) relatively or absolutely.
        (absolute|relative).
        For material-next distributions, relative, for anything else, you probably want absolute.`,
    }).argv;
}
