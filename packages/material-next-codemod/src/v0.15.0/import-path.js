...
const pathBaseSource = ['@material-next/core/src/', '@material-next/core/src/'];
const pathBasePackage = ['@material-next/core/lib/', '@material-next/core/'];
...
export default function transformer(file, api) {
  ...
  return j(file.source)
    ...
    .filter(path => {
      // Only consider Material-Next imports
      return path.value.source.value.indexOf('@material-next/core/') === 0;
    })
    .replaceWith(path => {
